...
// Define ad blocking rules for common ad domains.
const adBlockingRules = [
  ...
  // New rule: Block ajs-assets.ftstatic.com scripts
  {
    id: 10,
    priority: 1,
    action: { type: "block" },
 ...
 //Updates the dynamic rules based on whether ad blocking is enabled.
 //If enabled, the ad blocking rules are added; if disabled, they are removed.
 
function updateAdBlockingRules(enabled) {
  chrome.declarativeNetRequest.updateDynamicRules({
    // Always remove these rule IDs first
    removeRuleIds: adBlockingRules.map(rule => rule.id),
    // If enabled, add the rules; otherwise, add an empty list.
