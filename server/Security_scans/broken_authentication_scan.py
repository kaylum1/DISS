...
def check_broken_authentication(url):
    ...
    For file URLs (file://), it opens the file from disk.
    ...
    try:
        parsed = urlparse(url)
        # Determine how to get the HTML based on the scheme.
        if parsed.scheme == "file":
            # Remove "file://" and open the file.
            file_path = url[len("file://"):]
            with open(file_path, "r", encoding="utf-8") as f:
                html = f.read()
            # For file URLs, we assume the provided file is the login page,
            # so we use it directly (ignoring the "/login" appending logic).
            test_url = url  
        else:
            # For HTTP/HTTPS URLs, append "/login" if necessary.
            if "login" not in url.lower():
                test_url = url.rstrip("/") + "/login"
            else:
                test_url = url
