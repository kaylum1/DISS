...
def normalize_url(url: str) -> str:
    """
    Normalize the URL to its base form:
      - For file URLs, return only the file name.
      - For HTTP/HTTPS, keep scheme, netloc and a normalized path (without trailing slashes, defaulting to '/'),
        dropping query parameters and fragments.
    """
    parsed = urlparse(url)
...
# Set up Jinja2 templates (assumes templates are located in server/templates)
...
# Use an absolute path for the templates directory based on the file location.
